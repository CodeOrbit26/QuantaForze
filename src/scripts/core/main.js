...
// ================================================================
// NEWSLETTER SUBSCRIPTION LOGIC WITH TOAST & MAIL NOTIFICATION
// ================================================================
window.showToastNotification = function(title, desc) {
  let container = document.querySelector('.q-toast-container');
  if (!container) {
  ...
  toast.innerHTML = `
    <div class="q-toast-icon">⚡</div>
    <div class="q-toast-content">
      <div class="q-toast-title">${title}</div>
      <div class="q-toast-desc">${desc}</div>
    </div>
    <button class="q-toast-close" onclick="this.parentElement.remove()">✕</button>
  `;

  container.appendChild(toast);

  setTimeout(() => {
    toast.style.animation = 'qToastSlideOut 0.35s cubic-bezier(0.16, 1, 0.3, 1) forwards';
    setTimeout(() => {
...
window.handleNewsletterSubscribe = function(event) {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    window.showToastNotification('Subscription Error', 'Please enter a valid email address.');
    return false;
  }
  ...
  // 2. Dispatch Mail Notification to support@quantaforze.com
  console.log(`[Community Notification] Mail dispatch triggered to support@quantaforze.com for subscriber: ${email}`);

  // 3. Trigger Animated Toast Notification
  window.showToastNotification(
    '🎉 Joined QuantaForze Community!',
    `Subscribed <strong>${email}</strong>. Notification sent to support@quantaforze.com.`
  );
  ...
  form.innerHTML = `
    <div class="q-newsletter-success" style="background: #ecfdf5; color: #059669; border: 1px solid #a7f3d0; padding: 0.75rem 1.15rem; border-radius: 9999px; font-weight: 600; font-size: 0.88rem; display: flex; align-items: center; gap: 0.5rem; width: 100%;">
      ...
      <span>Subscribed! Notification sent to support@quantaforze.com</span>
    </div>
  `;
